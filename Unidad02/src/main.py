...
Ejecutar desde el Codespace con: cd src && python main.py
"""
...
y = df[TARGET]
top_features = seleccionar_features(X, y, k=K_FEATURES)
...
m = res['metricas']
log.info('%s: F1=%.4f Recall=%.4f AUC=%s',
nombre, m['f1'], m['recall'], m['roc_auc'])
log.info('Mejor modelo: %s | FN=%d | FP=%d',
mejor_nombre, errores['FN'], errores['FP'])
