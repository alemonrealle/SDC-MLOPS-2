...
Extrae las funciones de las Partes 2 y 3 del notebook original.
""
...
logger.info('%s: %d outliers [%.2f, %.2f]', col, n, lower, upper)
return df
def imputar_nulos(df, estrategias=None):
"""Imputa valores nulos segun estrategia por columna (Parte 2.3)."""
df = df.copy()
defaults = {
...
df[col].mean() if strategy=='mean' else
df[col].mode()[0])
df[col].fillna(val, inplace=True)
logger.info('%s: %d nulos imputados con %s', col, n, strategy)
return df
def dividir_y_balancear(X, y, test_size=0.2, tecnica="smote", random_state=42):
"""Division estratificada y balanceo (Partes 2 y 3 del notebook)."""
X_tr, X_te, y_tr, y_te = train_test_split(
X, y, test_size=test_size, random_state=random_state, stratify=y)
logger.info('Split: train=%d test=%d', len(y_tr), len(y_te))
sampler_map = {
...
logger.info('Balanceo %s: antes=%d despues=%d', tecnica, len(y_tr), len(y_bal))
return X_bal, X_te, y_bal, y_te